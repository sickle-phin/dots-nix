...
import { OpenWallpaper, Wallpaper, Monitor } from "./wallpapers.js";
...
function Top() {
	return Widget.Box({
		spacing: 8,
		vertical: true,
		children: [Volume(), Cpu(), Memory(), Battery(), Workspaces()],
	});
}
...
function Battery() {
	const value = Utils.merge(
		[battery.bind("percent"), battery.bind("available")],
		...
		children: [
			Widget.CircularProgress({
				...
				child: Widget.Icon({
					icon: battery
						.bind("available")
						.as((x) => `${x ? "battery" : "battery-missing"}`),
					size: 10,
				}),
			}),
		],
