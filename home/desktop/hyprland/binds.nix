...
in
{
  wayland.windowManager.hyprland.settings = {
    ...
    bind = [
      ...
      "$mod, A, exec, ${dms} ipc call widget toggle polyglot"
      ...
      "$mod, Print, exec, pkill wl-screenrec || uwsm-app -- ${getExe pkgs.wl-screenrec} -f \"${config.home.homeDirectory}/Videos/Screencasts/$(date +%Y%m%d_%H%M%S).mp4\""
    ...
    bindl = [
      ...
      ", XF86AudioPrev, exec, ${getExe pkgs.playerctl} position -5"
      ", XF86AudioNext, exec, ${getExe pkgs.playerctl} position +5"
    ];
