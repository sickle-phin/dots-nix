    ...
    settings = {
      ...
      bind =
        [
          ...
          "$mod, D, exec, ${setXCursor}; uwsm app -- fuzzel --config \"${config.xdg.configHome}/fuzzel/$(cat ${config.xdg.cacheHome}/hypr/theme).ini\""
          ...
          "$mod, V, exec, uwsm app -- cliphist list | fuzzel -d --config \"$XDG_CONFIG_HOME/fuzzel/$(cat $XDG_CACHE_HOME/hypr/theme).ini\" | cliphist decode | wl-copy"
          "$mod, Q, killactive"
